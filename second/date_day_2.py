...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QVBoxLayout, QTextEdit,
                             ...
                             QMessageBox, QStatusBar, QTabWidget, QWidget, QLabel)
...
class TextCanvas(QMainWindow):
    ...
    def addNewTab(self, filepath=None):
        ...
        if extension == 'pdf':
            try:
                ...
                text_edit.setReadOnly(False)  # Только для чтения

                # self.layout.addWidget(text_edit)

                # print(tab_name)
                # file_name = filename.split('/')[-1]
                index = self.tab_widget.addTab(text_edit, tab_name)
                self.tab_widget.setCurrentIndex(index)
            except Exception as e:
                print(f"Ошибка извлечения текста: {e}")
            # url = 'file:' + pathname2url(tab_name)
    ...
    def saveCurrentTab(self):
        ...
        current_widget = self.tab_widget.currentWidget()
        print('1 gde')
        # print('1', current_widget.text_edit.toPlainText())
        if not current_widget:
            print('2', current_widget.text_edit.toPlainText())
            return
        ...
        if current_widget.filepath:  # Если файл уже был открыт
            ...
            if filename:
                ...
                if tab_name.split('.')[1] == 'txt':
                    ...
                    print('1', tab_name, current_widget.text_edit.toPlainText())
                    # with open(filename, 'r', encoding='utf-8') as input_file:
                    #     print('2', tab_name)
                    text2 = current_widget.text_edit.toPlainText()
                    # text = ExtractText().extraction(tab_name)
                    # for element in current_widget.text_edit.toPlainText().simpl('\n'):
                    #     text2 += element
                    #     print(text2)
                    # WritingText().writing(CreateFile().creation(tab_name), text)
                    WritingText().writing(tab_name, text2)
                    # text = ExtractText().extraction(tab_name)
                    # self._saveToFile(filename, text)
...
#             original_path = self.file_paths.get(current_index)
#             if original_path:
#                 # Предлагаем сохранить с тем же именем или новым
#                 save_filename, _ = QFileDialog.getSaveFileName(
#                     self, "Сохранить изменения", original_path, "Text Files (*.txt);;PDF Files (*.pdf)"
#                 )
#                 if save_filename:
...
#                     f.write(text)
#         elif save_filename.endswith('.pdf'):
#             # Для сохранения в PDF нужно создать новый PDF
#             self.save_as_pdf(save_filename, text)
