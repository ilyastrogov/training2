...
class ForDict1:
    ...
    def sum_number(self):
        string_for_search2 = ''
        string_for_search1 = ('Ярославский, вокзал шибал свежестью '
                             'и тепловозной гарью. После прокисшего '
                             'плацкартного пара, после прокуренного '
                             'железа тамбуров, подслащенного мочой, – '
                             'тут воздух был слишком огромный: кислорода '
                             ...
                             'чтобы мимо цепких глаз, чтобы не примагнититься к ним. Стал думать '
                             'ни о чем, чтобы ничем не пахнуть.')
        # numbers_string = string_for_search
        temp_word = ''
        index = -1
        count_word = 0
        space = 0
        for e in string_for_search1:
            index += 1
            if e.isalpha():
                space = 0
                temp_word += e
                ...
                if count_word % 2 == 0:
                    # space = 0
                    string_for_search1 = (string_for_search1[:index - (len(temp_word))] +
                                         string_for_search1[index:])
                    ...
                    # space = 0
                    temp_word = ''
        print('str1', string_for_search1)
        string_for_search2 = ' '.join(string_for_search1.split())
        string_for_search2.replace(' ', '')
        return 'str2', string_for_search2


class PrintPosition1(ForDict1):
